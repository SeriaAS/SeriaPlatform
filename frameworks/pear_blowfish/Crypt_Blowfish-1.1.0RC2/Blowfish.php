 ...
 *  ===> Modified to include by J-E P to let it work without PEAR(!)
 */
    ...
    function &factory($mode = 'ecb', $key = null, $iv = null, $engine = CRYPT_BLOWFISH_AUTO)
    {
        ...
        switch ($engine) {
            case CRYPT_BLOWFISH_PHP:
                ...
                require_once(dirname(__FILE__).'/Blowfish/' . $mode . '.php');
                $crypt = new $class(null);
                break;

            case CRYPT_BLOWFISH_MCRYPT:
                include_once(dirname(__FILE__).'/Blowfish/MCrypt.php');
