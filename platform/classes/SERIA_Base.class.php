...
$trace");
		...
		static function getParam($name, $forceFetch=false)
		{
			if(!$forceFetch && isset($GLOBALS[SERIA_PREFIX.'_params'][$name]))
			{
				return $GLOBALS[SERIA_PREFIX.'_params'][$name];
			}
			...
			try {
				...
				$phpBuilder->addVariable(array('GLOBALS',SERIA_PREFIX.'_params',$name), $res);
				$phpBuilder->save();
				$GLOBALS[SERIA_PREFIX.'_params'][$name] = $res;
				return $res;
			} catch (PDOException $e) {
				/*
				 * If we are in install mode, we handle a missing table by returning false
				 */
				if ($e->getCode() == '42S02')
					return null;
				throw $e;
		...
		{
			try {
				SERIA_Base::db()->exec("REPLACE INTO {params} (name, value) VALUES (:name, :value)",array('name' => $name, 'value' => $value), true);
				$phpBuilder = SERIA_PhpBuilder::createObject(SERIA_PRIV_ROOT.'/SERIA_PLATFORM.php');
				$phpBuilder->addVariable(array('GLOBALS',SERIA_PREFIX.'_params',$name), $value);
				$phpBuilder->save();
				$GLOBALS[SERIA_PREFIX.'_params'][$name] = $value;
				return true;
		...
		{
			try {
				SERIA_Base::db()->exec("INSERT INTO {params} (name, value) VALUES (:name, :value)",array('name' => $name, 'value' => $value), true);
				$phpBuilder = SERIA_PhpBuilder::createObject(SERIA_PRIV_ROOT.'/SERIA_PLATFORM.php');
				$phpBuilder->addVariable(array('GLOBALS',SERIA_PREFIX.'_params',$name), $value);
				$phpBuilder->save();
				return true;
		 ...
		 * @return unknown_type
		 */
		static function setParamIfEqualTo($name, $value, $compare)
		{
			try {
				if (SERIA_Base::db()->exec('UPDATE {params} SET value = :value WHERE name = :name AND value = :compare', array('name' => $name, 'value' => $value, 'compare' => $compare), true) === 1)
				{
					$phpBuilder = SERIA_PhpBuilder::createObject(SERIA_PRIV_ROOT.'/SERIA_PLATFORM.php');
					$phpBuilder->addVariable(array('GLOBALS',SERIA_PREFIX.'_params',$name), $value);
					$phpBuilder->save();
