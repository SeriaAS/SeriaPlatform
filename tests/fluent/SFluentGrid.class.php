<?php
	class SFluentGrid
	{
		...
		function __construct(SFluentQuery $query)
		{
			$this->_query = $query;
		}
		...
		function output($columnSpec, $templateOrCallback = NULL)
		{
			...
			foreach($columnSpec as $fieldName => $spec)
			{
				...
				else
				{
					if(!isset($fieldSpec[$fieldName]['caption']))
						throw new SException('Property "caption" not specified for attribute "'.$fieldName.'" '.$this->_query->className);
			...
			else if($templateOrCallback !== NULL)
			{
				...
				{
					throw new SException('Most likely you have not specified all used fields in the ::getFieldSpec() method of your FluentObject. The failed property was "'.$word.'".');
				}
