<?php
	...
	if (($id = $_GET['id']) && is_numeric($id)) {
		$object = SERIA_FtpFileProtocols::find($id);
		if (!$object) {
	...
	SERIA_Base::redirectTo(SERIA_HTTP_ROOT . '/seria/settings/ftpservers/addserver.php?edit=' . (int) $_GET['ftp_server_id'] . '#protocols');
?>